...
const MissionsPage: React.FC<MissionsPageProps> = ({ onShowRelations }) => {
    ...
    const [filters, setFilters] = useState<{ rattachementId: string, statut: string }>({ rattachementId: 'all', statut: 'all' });

    const filteredMissions = useMemo(() => {
        return missions.filter(m =>
            (m.nom.toLowerCase().includes(searchTerm.toLowerCase())) &&
            (filters.rattachementId === 'all' || m.rattachementId === filters.rattachementId) &&
    ...
    const getRattachementName = (mission: Mission) => {
        if (mission.rattachementType === 'Entite') {
            return entites.find(e => e.id === mission.rattachementId)?.nom || 'N/A';
        }
    ...
    return (
        <div className="flex h-[calc(100vh-150px)]">
            <div className="flex-1 flex flex-col min-w-0">
                <div className="p-4 border-b bg-white flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={() => handleOpenModal()} className="flex items-center space-x-2 bg-blue-600 text-white px-3 py-1.5 rounded-lg hover:bg-blue-700 text-sm font-medium"><Plus className="h-4 w-4" /><span>Nouvelle Mission</span></button>
                    </div>
                </div>
                <div className="p-2 border-b bg-white flex flex-wrap items-center gap-2">
                    ...
                    <select onChange={e => setFilters(f => ({...f, rattachementId: e.target.value}))} className="border rounded-lg py-1.5 px-2 text-sm"><option value="all">Tous les rattachements</option>{entites.map(e => <option key={e.id} value={e.id}>{e.nom}</option>)}{postes.map(p => <option key={p.id} value={p.id}>{p.intitule}</option>)}</select>
                ...
                <div className="flex-1 overflow-auto p-4 bg-gray-50">
                    <div className="bg-white border rounded-lg">
                        <table className="min-w-full">
                            <thead className="bg-gray-50"><tr>{['Titre', 'Rattachement', 'Responsable', 'Statut', 'Actions'].map(h => <th key={h} className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">{h}</th>)}</tr></thead>
                            <tbody className="divide-y divide-gray-200">
                                ...
                                <tr key={mission.id} onClick={() => setSelectedMission(mission)} className={`hover:bg-gray-50 cursor-pointer ${selectedMission?.id === mission.id ? 'bg-blue-50' : ''}`}>
                                    ...
                                    <td className="px-4 py-2 text-sm text-gray-500">{postes.find(p => p.id === mission.responsablePosteId)?.intitule || 'N/A'}</td>
            ...
            {selectedMission && <MissionDetailPanel mission={selectedMission} onClose={() => setSelectedMission(null)} onEdit={handleOpenModal} onShowRelations={onShowRelations} />}
