...
import type { Actif, Personne, Poste, Risque, Controle } from '../types';
...
const ActifDetailPanel: React.FC<ActifDetailPanelProps> = ({ actif, onClose, onEdit, onAddMaintenance }) => {
    ...
    const { personnes, postes, risques, controles, actifs } = data as { personnes: Personne[], postes: Poste[], risques: Risque[], controles: Controle[], actifs: Actif[] };
    
    const owner = personnes.find(p => p.id === actif.proprietaireId) || postes.find(p => p.id === actif.proprietaireId);
    const manager = personnes.find(p => p.id === actif.gestionnaireId) || postes.find(p => p.id === actif.gestionnaireId);
    
    const linkedRisks = risques.filter(r => actif.lienRisqueIds?.includes(r.id));
    const linkedControls = controles.filter(c => actif.lienControleIds?.includes(c.id));
    ...
    return (
        <div className="w-full max-w-lg bg-white border-l shadow-lg flex flex-col h-full absolute right-0 top-0 md:relative animate-slide-in-right">
            ...
            <div className="flex-grow p-4 overflow-y-auto bg-gray-50/50">
                ...
                {activeTab === 'tree' && <ActifTree rootActifId={actif.id} allActifs={actifs} />}
                            ...
                            <div className="space-y-2">
                                {actif.maintenanceLogs?.map(log => (
                                    <div key={log.id} className="p-2 bg-white border-b">
                                        ...
                                        <p className="text-xs text-gray-500">Technicien: {personnes.find(p=>p.id === log.technicien)?.nom}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
