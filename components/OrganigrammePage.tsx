...
import { Network, ZoomIn, ZoomOut, Minimize2, Download, Image, FileText, User, Repeat } from 'lucide-react';
...
const Toolbar: React.FC<{
...
}> = ({ onExportPNG, onExportPDF, onFindMe, onToggleLayout, layoutDirection }) => {
    ...
    return (
        <div className="absolute top-4 left-4 z-10 bg-white/80 backdrop-blur-sm p-2 rounded-lg shadow-lg border flex items-center gap-2">
            ...
            <button onClick={() => fitView()} title="Ajuster à la vue" className="p-2 hover:bg-gray-100 rounded-md"><Minimize2 className="h-5 w-5"/></button>
            ...
            <div className="relative group">
                ...
                <div className="absolute left-0 top-full mt-1 w-40 bg-white border rounded-lg shadow-lg opacity-0 group-hover:opacity-100 invisible group-hover:visible transition-opacity z-10">
                    <button onClick={onExportPNG} className="w-full text-left flex items-center gap-2 px-3 py-2 hover:bg-gray-100 text-sm"><Image className="h-4 w-4 text-blue-600"/>Exporter en PNG</button>
                    <button onClick={onExportPDF} className="w-full text-left flex items-center gap-2 px-3 py-2 hover:bg-gray-100 text-sm"><FileText className="h-4 w-4 text-red-600"/>Exporter en PDF</button>
                </div>
            </div>
        </div>
    );
...
const OrganigrammePageContent: React.FC<OrganigrammePageProps> = ({ onShowRelations }) => {
    ...
    const { setCenter, zoomTo, getNodes } = useReactFlow();
