...
const NewsPage: React.FC = () => {
    ...
    const handleOpenModal = (actu?: Actualite) => {
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-auto p-4 bg-gray-50">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredNews.map(actu => {
                        ...
                        return (
                        <div key={actu.id} className="bg-white rounded-lg shadow-sm border flex flex-col group">
                            <img src={actu.imageURL} alt={actu.nom} className="w-full h-40 object-cover rounded-t-lg bg-gray-200" />
                            ...
                            <div className="p-2 bg-gray-50/50 rounded-b-lg opacity-0 group-hover:opacity-100 transition-opacity flex justify-end gap-2">
                                <button onClick={() => handleOpenModal(actu)} className="p-1 hover:bg-gray-200 rounded"><Edit className="h-4 w-4 text-blue-600"/></button>
                                <button onClick={() => handleDeleteNews(actu.id)} className="p-1 hover:bg-gray-200 rounded"><Trash2 className="h-4 w-4 text-red-600"/></button>
                            </div>
                        </div>
                    )})}
                </div>
            ...
            <NewsFormModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={handleSaveNews} actualite={editingNews} />
        </div>
    );
};
